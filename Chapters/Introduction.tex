...
(TODO : REWRITE THE FIRST PARAGRAPH)
...
\begin{align*}
    ...
    \psi_G(e_8) = ae
\end{align*}
An equivalent way to define a graph would be through the \emph{incidence matrix} $M_G = (m_{ve}) $ with $m_{ve}$ the number of times the vertex $v$ and the edge $e$ are incident. So $m_{ve}$ can take the values 0 (not incident), 1 or 2 ( $e$ is a loop ).
...
The latter is usually the most useful version as typically a graph will have less vertices than edges, the adjacency matrix will be much smaller to write ( hence to store in a computer ) and it usually gives all the information needed to study a graph. 
It is interesting to note that an adjacency matrix is real and hermitian, thus all of its eigenvalues are real and the study of theirs distribution is a common topic in graph theory.
\newline
...
In graph theory a graph can be represented in many different ways and then it can be really non trivial to know if two graphs with different labelling are the same. More formally, for a same graph, we will call the set of permutation of the labelling that doesn't change the structure of the graph, it's automorphism group, denoted $Aut(G)$ and $aut(G)$ it's cardinal.
\newline
...
There are two famous kinds of path, the \emph{hamiltonian path}, it is a path that includes all vertices of $G$. Analogously, an \emph{Eulerian path} is a path in which all edges are used only once.
\newline
