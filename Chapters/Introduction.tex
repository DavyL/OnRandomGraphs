...
$\mathcal{G}_n$ will define a probability space of random graphs on $n$ vertices and $\mathbb{G}_n$ will be a random (graph) variable in a specified space of random graphs.
The distinction between $\mathcal{G}$ and $\mathbb{G}$ is not necessarily done.
We will say that a property is satisfied with high probability if the probability is tending to one as $n$ goes to infinity.
...
\begin{figure}[h]
	...
	\caption{The corresponding doubly-rooted tree of \eqref{eq:functionJoyal}, the "Start" root is vertex 1, the "End" root is vertex 4.}
\end{figure}
