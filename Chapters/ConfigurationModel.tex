...
\end{proof}
It is interesting to note through the following theorem, which we will not prove, that under certain conditions, a similar behaviour appears between the binomial model $\mathcal{G}_{n,p}$ and the generalized binomial $\mathcal{G}_{n, \bold{P}}$ at the connectivity threshold.
\begin{theorem}
...
Hence, the fraction of vertices of degree $k$ is $p_k = Ck^{-\beta}$, with $C$ the normalization constant and typically most of the observed networks which can be represented by a power-law distribution have $\beta$ between 2 and 3.
We are here interested in constructing and investigating graphs following a specified degree distribution.
We will use the approach of Newman, Strogats and Watts (TODO: ADD REF).
\newline
...
\newline
The approach from Newman, Strogatz and Watts makes an extensive use of generating functions defining the degree sequence, we will here only proof some simple results and give some interesting results in order to give an insight of the efficiency of the model to mimic real-world phenomenoms.
\newline
...
\begin{equation}\label{eq:G0def}
	G_0(x) = \sum_k p_k x^k
\end{equation}
We may here observe that \eqref{eq:G0def} satisfies a condition of normalization with $G_0(1) = 1$.
As in the following we will base our approach on this generating function it is interesting to note that this normalization condition is the only we require.
...
\newline
Let's remark that if we take an edge chosen at random, then it arrives at a vertex with a probaility that is proportional to its degree.
Hence, such a vertex has a probability distribution on its degree proportional to $kp_k$. 
Which give that the normalized generating function of the degree of vertices that we arrive at by a randomly chosen edge is:
...
\begin{equation}
	G_1(x) = G_0(x).
\end{equation}
This means that in the case of a random graph following a Poisson distribution on its degree has the same distribution of outgoing edges at a vertex whether we arrived by a randomly chosen edge or if we picked a vertex at random.
...
We consider for the moment that there is no giant component with the degree distribution we investigate, removing this component growing linearly in $n$ allows us to set a normalization condition on $H_1$ such that $H_1(1) = 1$.
...
\end{align}
With $H_1(1) = 1$ we have $s = 1 + H_1'(1)G_0'(1)$.
...
\end{equation}
Replacing the above in \eqref{eq:sH0} we obtain
...
\newline
As what we did above would not have worked in the event $G_'(1) > 1$ we will here get an answer using the fact that an analysis of \eqref{eq:H1} gives that $H_1(1)$ is a fixed point of $G_1$.
