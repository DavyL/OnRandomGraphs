...
Above we have seen a generalized binomial model which changed the probability distribution on the set of all edges, now we will describe models in which we change the edge set}
...
\newline
Hence, an important part of research has been studying random bipartite graph that we can naturally describe the generalized model as follows:
...
This raises interest in studying the random generalized binomial model through the point of view of the generalized model.
...
Note that the following results are also formally true ( in the ring of formal power series ?? (TODO: ASK)) if there is a giant component as long as we do not consider that $H_1$ is not normalized.
