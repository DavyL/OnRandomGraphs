...
Now that we know that the diameter is spreading on two values we are interested in restricting it on only one value. 
...
\section{Generalized Erd\H{o}s-R\'enyi model}
...
\begin{proof}[Proof of Corollary \ref{th:proofPoissonP}]
	...
	When we have $k \leq \frac{n}{\log n}$ then \eqref{eq:expNQ} is uniformly bounded which gives uniform convergence using the dominated convergence theorem.
	...
	Hence, 
	\begin{equation}
		1 + \lim_{n\to \infty} \sum_{k=1}^{\lfloor n/2 \rfloor} \frac{1}{k!} (nq^{n-1-k})^k = e^{e^{-c}}
	\end{equation}
