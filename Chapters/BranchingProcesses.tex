...
\begin{proof}
	...
	\end{equation}
	Each of these $Z_n(j)$ is a GW process independent from the others and for each we can then construct $M_n(j)$ as in \eqref{martinGW}.
...
This will be the point of this chapter, studying links between random graphs and branching process, so we will consider that our probability $p$ has to be small enough such that there is not a single connected component. From \eqref{th:isolcomp} we will consider that $p=\frac{\lambda}{n}$.
\newline
...
\begin{proof}[Proof of \eqref{th:uplambda}]
	...
	Using \eqref{th:Atlaw} we obtain that $|A_t| \sim \text{Bin}(n-1, 1-(1-p)^t) - (t-1)$, so 
	\begin{equation}
	...
	\end{equation}
	Replacing the moment generating function of the binomial with it's value \eqref{binMGF}, we obtain 
	\begin{equation}
...
\end{equation}
Applying the theorem above \eqref{th:uplambda} we immediately have the following result.
...
\begin{proof}
	...
	\end{equation}
	And using \eqref{th:uplambda}\footnote{The inequality being strict the -1 appears}  we obtain
	\begin{equation}
		\chi_{k_n}(\lambda) \leq \sum_{t=k_n}^{n} e^{-I_{\lambda}(t-1)} \leq \frac{e^{-(k_n -1)I_{\lambda}}}{1-e^{-I_{\lambda}}} = \mathcal{O}(n^{-a I_{\lambda}})
	\end{equation}
	And now we need to find a lower bound on the expectation of $Z_{\geq k}$ So we will make use of \eqref{eq:nexplambda} to obtain the formula of the expectation, 
	\eqref{th:lowbin} to obtain a lower bound and \eqref{th:poibin} to convert our lower bound in the Poisson distribution.
	So we have
	\begin{equation}
...
\end{proof}
Now, simply combining the previous results \eqref{lemmalambdainf} and \eqref{lemmalambdasup} wehave our proof of our main theorem in this section \eqref{th:subCritCV}.
