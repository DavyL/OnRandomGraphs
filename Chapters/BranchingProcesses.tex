...
Typically in a branching process we start with one individual, consider it will create a number of individuals through his lifetime, and those individuals will create themselves a number of individuals through their lifetime, and so on. 
...
See Figure \ref{fig:BFS} for a visual representation of the BFS.
Notice that we have:
...
So we consider $X = \{X_i\}_i^N$ our sequence of i.i.d. random variables where each $X_i$ follows a Bernoulli of parameter $p$ in order to get an Erd\H{o}s-Renyi random graph process.
...
A brief introductory review on the phase transition of Erd\H{o}s-Renyi random graphs and other settings in which a similar phase transition can be observed (bond percolation, $d$-regular random graphs, specified degree sequence, ...) can be found in \cite{Spencer09}.
