...
The proof will make an extensive use of the depth-first seach algorithm (TODO : DESCRIBE IT SOMEWHERE ), at each step of the algorithm, when it is searching for a neighbour it is simply following a Bernoulli random variable of parameter $p$.
...
\begin{lemma}\label{lemmaN0}
	Let $p = \frac{1+\epsilon}{n}$ and $N_0 = \frac{\epsilon n^2}{5}$. Then, 
	\begin{equation}
...
\begin{proof}
	...
	TODO : NOT CLEAR WHY 
	\newline
	We are then sure from the previous argument that $|E_{N_0}| < \frac{n}{3}$ and from \ref{eq:At} $|A_{N_0}| \leq 1 + N_0$, then $U_{N_0} \neq \emptyset$. 
